...
async function resolveOwner(req: Request): Promise<{ ownerId: string; userId: string } | null> {
  ...
  const authClient = createClient(SUPABASE_URL, SUPABASE_ANON_KEY, { db: { schema: "onlineclass_customization" } });
  const { data, error } = await authClient.auth.getClaims(token);
  const userId = (data as any)?.claims?.sub;
  if (error || !userId) return null;

  const admin = createClient(SUPABASE_URL, SERVICE_KEY, { db: { schema: "onlineclass_customization" } });
...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "upload") {
      ...
      const publicUrl = `${ENDPOINT}/${bucket}/${key}`.replace(/^https?:\/\/[^\/]+/, "https://supabase.buildstart.io");
